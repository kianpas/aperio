...
public interface UserCouponRepository extends JpaRepository<UserCoupon, Long> {
        ...
        @Query("SELECT uc FROM UserCoupon uc LEFT JOIN FETCH uc.coupon c " +
                        ...
                        "AND (:startDate IS NULL OR uc.coupon.startAt >= :startDate) " + // 시작일 조건
                        "AND (:endDatePlusOne IS NULL OR uc.coupon.startAt < :endDatePlusOne) " + // 종료일 조건
        ...
        @Query(value = """
                        ...
                        AND c.startAt     <= :now
                        AND c.endAt       >= :now
                        AND (:startDate IS NULL OR uc.coupon.startAt >= :startDate)
                        AND (:endDatePlusOne IS NULL OR uc.coupon.startAt < :endDatePlusOne)
                        ORDER BY uc.createdAt DESC
                        """, countQuery = """
                         SELECT COUNT(uc)
                            FROM UserCoupon uc
                            ...
                            WHERE uc.user.id   = :userNo
                              AND c.startAt     <= :now
                              AND c.endAt       >= :now
                              AND (:startDate      IS NULL OR c.startAt >= :startDate)
                              AND (:endDatePlusOne IS NULL OR c.startAt <  :endDatePlusOne)
                        """)
